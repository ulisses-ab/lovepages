...
export default function PageBgWrapper({ settings, className = '', style = {}, children, viewportFixed = false }) {
  ...
  if (settings?.bgShader) {
    return (
      <div className={className} style={{ position: 'relative', ...style }}>
        ...
        <BgEffect effect={settings?.bgEffect} variant={settings?.bgEffectVariant} color={settings?.bgEffectColor} options={settings?.bgEffectOptions} pos={bgPos} />
  ...
  if (viewportFixed && (settings?.bgImage || settings?.bgColor)) {
    ...
    return (
      <div className={className} style={{ position: 'relative', ...style }}>
        <div style={bgLayer} />
        <BgEffect effect={settings?.bgEffect} variant={settings?.bgEffectVariant} color={settings?.bgEffectColor} options={settings?.bgEffectOptions} pos="fixed" />
        <div style={contentStyle}>{children}</div>
      </div>
    )
  }
  ...
  return (
    <div className={className} style={{ position: 'relative', ...bgStyle, ...style }}>
      <BgEffect effect={settings?.bgEffect} variant={settings?.bgEffectVariant} color={settings?.bgEffectColor} options={settings?.bgEffectOptions} pos="absolute" />
      <div style={contentStyle}>{children}</div>
    </div>
  )
}
